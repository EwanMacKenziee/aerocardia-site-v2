...
- **Championship editions**: 16 of 17 country colourways now have real renders. Netherlands and Belgium still use the original subtle accent-bezel style (`assets/img/renders/`). Everyone else — Great Britain, Italy, France, Germany, Spain, Colombia, Ecuador, Eritrea, Ethiopia, Canada, Norway, Australia, United States, UAE — uses the full color-block shell style (`assets/img/editions/`, sourced from "Flags other views.zip", "last flags", and "UAE flag.zip"). GB was switched over from the old cobalt accent render to a literal Union Jack full-shell render by request. Note the two styles look different side by side — that's a known tradeoff, not a bug. Still missing: **Mexico** — the only one of 17 with no render yet, so it still points to `assets/img/editions/mexico.png`, which doesn't exist, and the viewer falls back to "Render coming soon."
